...
BEGIN_AISTATE_MAP( CAIAggroNormal, CAIInterface )

	ON_STATE(STATE_INIT, &CAIAggroNormal::StateInit)
	ON_STATE(STATE_IDLE, &CAIAggroNormal::StateIdle)
	ON_STATE(STATE_WANDER, &CAIAggroNormal::StateWander)
	ON_STATE(STATE_EVADE, &CAIAggroNormal::StateEvade)
	ON_STATE(STATE_RAGE, &CAIAggroNormal::StateRage)
	ON_STATE(STATE_RUNAWAY, &CAIAggroNormal::StateRunaway)
	ON_STATE(STATE_STAND, &CAIAggroNormal::StateStand)
	ON_STATE(STATE_PATROL, &CAIAggroNormal::StatePatrol)
	ON_STATE(STATE_RAGE_PATROL, &CAIAggroNormal::StateRagePatrol)
...
#ifndef __MEM_TRACE
	#ifdef __VM_0819
	MemPooler<CAIAggroNormal>*	CAIAggroNormal::m_pPool		= new MemPooler<CAIAggroNormal>( INITIAL_AI_MONSTER, "CAIAggroNormal" );
	#else	// __VM_0819
	MemPooler<CAIAggroNormal>*	CAIAggroNormal::m_pPool		= new MemPooler<CAIAggroNormal>( INITIAL_AI_MONSTER );
	#endif	// __VM_0819
#endif	// __MEM_TRACE
#endif	// __VM_0820
...
CAIAggroNormal::CAIAggroNormal( CObj* pObj ) : CAIInterface( pObj )
{
	Init();
}
...
BOOL CAIAggroNormal::IsInRange( D3DXVECTOR3& vDistant, FLOAT fRange )
{
#ifdef __JEFF_11_6
	vDistant.y	= 0.0f;
#endif	// __JEFF_11_6
...
BOOL CAIAggroNormal::MoveProcessIdle( const AIMSG & msg )
{
...
#if __VER < 9	//__AI_0509
		else if( m_dwIdTarget != NULL_ID )	// 놀고있던 중에 전에 ?던 적이 있으면 재공격
		{
	...
	else
	{
		...
		if( pProp->m_nScanJob != -1 )
		{
			CMover* pTarget = NULL;
			int nAttackFirstRange = pProp->m_nAttackFirstRange;
			if( nAttackFirstRange > 80 || nAttackFirstRange <= 0 )
			{
...
NEXT2:
	for( int i = 0; i < pMoverProp->m_nCallHelperMax; i++ ) // VELO!!!!
	{
		if( pParty  )
		{
			if( nCountPlayer )
...
DWORD CAIAggroNormal::GetAtkRange( DWORD dwAtkMethod )
{
	...
	if( pItemProp == NULL )
	{
		Error( "CAIAggroNormal::GetAtkRange : ItemProp->dwAtk%d이 NULL이다. (%s)", dwAtkMethod, GetMover()->GetName() );
	}
#endif
	return pItemProp->dwAttackRange;
}

ItemProp* CAIAggroNormal::GetAtkProp( DWORD dwAtkMethod )
{
...
void CAIAggroNormal::DoAttack( DWORD dwAtkMethod, CMover* pTarget )
{
	...
	if( pItemProp == NULL )
	{
		Error( "CAIAggroNormal::DoAttack : %s %d %d", pMover->GetName(), dwAtkMethod, dwAtkItem );
		return;
	}
...
BOOL CAIAggroNormal::MoveProcessRage( const AIMSG & msg )
{
...
#endif	// __AI_0509
	else if( pTarget->GetType() == OT_MOVER && ((CMover *)pTarget)->IsMode( TRANSPARENT_MODE ) )		// ?고 있던 타겟이 투명화 상태가 되면 
	{
#if __VER >= 14 // __INSTANCE_DUNGEON
		if( CInstanceDungeonHelper::GetInstance()->IsInstanceDungeon( pWorld->GetID() ) )
		{
	...
	else
	{
		...
		if( m_bRangeAttack )	// 원거리 공격으로 결정됐음.
		{
			...
			{
				Error( "CAIAggroNormal::MoverProcessRage : 범위침범 %d", nTargetJob );
				nTargetJob = 0;
			}
...
BOOL CAIAggroNormal::StateRage( const AIMSG & msg )
{
	...
	OnMessage( AIMSG_PROCESS ) 
		MoveProcessRage( msg );
	///////////////////////////////////////////////////////////////////////////////////////////////////
	OnMessage( AIMSG_SKILLSUPPORT )
		ItemProp *pSkillProp = prj.GetSkillProp( msg.dwParam2 ); // skill prop
		AddSkillProp *pAddSkillProp = prj.GetAddSkillProp ( msg.dwParam2); // addskillprop
		CMover* pCaster	= prj.GetMover( msg.dwParam1 ); // caster
		CMover* pCasterTarget = prj.GetMover( msg.dwParam3 ); // casters target
		CMover* pCurrentTarget = prj.GetMover ( m_dwIdTarget ); // current target
		int nChance = 200;

		if ( pCasterTarget == pCurrentTarget && pCaster != pCasterTarget )
		{
	...
	OnMessage( AIMSG_DAMAGE )
		...
		CMover* pAttacker	= prj.GetMover( msg.dwParam1 );  
		CMover* pCurrentTarget = prj.GetMover ( m_dwIdTarget );
		int nChance = 100;
		if (pCurrentTarget != pAttacker )
		{
			if ( msg.dwParam3 == 0 )
			{
				if ( pCurrentTarget->m_pActMover->IsMove() )
					nChance /= 2;
				if ( xRandom ( 0 , nChance ) <= 25 )
				{	
					MoveToDst( msg.dwParam1 );
				}
			}
			else if( msg.dwParam3 == 1 )
			{
				MoveToDst( msg.dwParam1 );
			}
			else if( msg.dwParam3 == 2 )
			{
				ItemProp *pSkillProp = prj.GetSkillProp( msg.dwParam2 ); // skill prop
				AddSkillProp *pAddSkillProp = prj.GetAddSkillProp ( msg.dwParam2); // addskillprop
				if ( xRandom ( 0, nChance ) <= 30 ) // 30%
				{	
					MoveToDst( msg.dwParam1 );
				}
			}
